...
public class JavaFileUtils {
     ...
     *
     * @param ti        表信息
     * @param convertor 数据类的转化器
     * @return java类的源代码
     */
    private static String createJavaSrc(TableInfo ti, TypeConvertor convertor) {
        ...
        for (ColumnInfo c : map.values()) {
            lists.add(createFieldGetSetSRC(c, convertor));
        }
...
//		System.out.println(jfgs);
        ...
        for (TableInfo ti : map.values()) {
            createJavaPOFile(ti, MysqlTypeConvertor.getInst());
        }
